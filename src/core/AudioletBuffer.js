...
class AudioletBuffer {
  ...
  /**
   * Set the data in a section of the buffer by copying data from a second
   * buffer
   *
   * @param {AudioletBuffer} buffer The buffer to copy data from.
   ...
   */
  setSection(buffer, length, inputOffset, outputOffset) {
   ...
   * @param {Number} [outputOffset=0] An offset to write data to.
   */
  addSection(buffer, length, inputOffset, outputOffset) {
    inputOffset = inputOffset || 0;
    outputOffset = outputOffset || 0;
    var numberOfChannels = buffer.numberOfChannels;
    for (var i = 0; i < numberOfChannels; i++) {
   ...
   */
  resize(numberOfChannels, length, lazy, offset) {
      offset = offset || 0;
      // Local variables
  ...
  /**
   * Remove data from the beginning of the buffer, placing it in a second
   * buffer.
   *
   * @param {AudioletBuffer} buffer The buffer to move data into.
   */
   ...
   * @param {Function} [callback] Function called if the file loaded
   * sucessfully.
   */
  load(path, async, callback) {
